...
class DebtsBloc extends Bloc<DebtsEvent, DebtsState> {
  ...
  DebtsBloc(this._repo) : super(const DebtsState()) {
    ...
    on<DebtStateUpdated>(_onStateUpdated);
  }
      ...
      (Debt r) {
        ...
        try {
          ...
          final double oldPaidPct =
              (oldDebt != null && oldDebt.originalAmount > 0)
          ...
          if (oldPaidPct < 100 && newPaidPct >= 100) {
            notif.local.showNotification(
              ...
              body:
                  '¡Felicidades! Liquidaste completamente la deuda "${r.name}" (${CurrencyHelper.symbol}${r.originalAmount.toStringAsFixed(0)}). ¡Eres libre! 🎊',
  ...
  void _onStateUpdated(DebtStateUpdated event, Emitter<DebtsState> emit) {
    emit(
      state.copyWith(
        items:
            state.items
                .map((Debt b) => b.id == event.debt.id ? event.debt : b)
                .toList(),
      ),
    );
  }
