...
class ProfileDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<ProfileBloc>(
      create: (_) => ProfileBloc(getIt<ProfileBackendRepository>())..add(ProfileLoadMe()),
    ...
    return BlocBuilder<ProfileBloc, ProfileState>(
      builder: (BuildContext context, ProfileState state) {
        ...
        final String email = (state.info?.email != null && state.info!.email.isNotEmpty)
            ? state.info!.email
            : (authUser?.email ?? '');
        final String phoneNumber = (state.info?.phoneNumber != null && state.info!.phoneNumber!.isNotEmpty)
            ? state.info!.phoneNumber!
            : (authUser?.phoneNumber != null && authUser!.phoneNumber!.isNotEmpty ? authUser.phoneNumber! : 'No especificado');
                          ...
                          ? (photoUrl.contains('://')
                              ? ClipOval(
                                  child: Image.network(
                                    photoUrl,
                                    fit: BoxFit.cover,
                                    errorBuilder: (_, __, ___) => _buildInitialsAvatar(context, fullName),
                                  ),
                                )
                              : Center(
                                  child: Text(
                        ...
                        onPressed: () {
                          ...
                          Navigator.push<void>(
                            ...
                            MaterialPageRoute<void>(
                              builder: (BuildContext context) => BlocProvider<ProfileBloc>.value(
                                value: bloc,
